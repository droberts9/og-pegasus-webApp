...
angular.module('rt.popup', [])
    .factory('Popup', function ($window, $document, $timeout, $compile, $parse) {
        ...
        function loseFocus(e) {
            ...
            if ( (mouseDownInsidePopup === null || mouseDownInsidePopup === false) && openedPopup && !angular.element.contains(openedPopup.el[0], e.target)) {
                mouseDownInsidePopup = null;
                hidePopup();
            }
        }
        ...
        function checkMouseDown(e) {
            if (openedPopup) {
                mouseDownInsidePopup = angular.element.contains(openedPopup.el[0], e.target);
            }
        }
        ...
        function fixPosition(scope, anchor, element, arrow, options) {
            ...
            element.css({
                top: angular.isDefined(popupPosition.top) ? popupPosition.top + 'px' : 'initial',
                left: angular.isDefined(popupPosition.left) ? popupPosition.left + 'px' : 'initial',
                right: angular.isDefined(popupPosition.right) ? popupPosition.right + 'px' : 'initial',
                display: 'block',
                maxHeight: maxHeight
            });
        ...
        function makePopup(anchor, scope, options) {
            ...
            // Add arrow
            var arrow = angular.element('<div />', { 'class': 'arrow' });
